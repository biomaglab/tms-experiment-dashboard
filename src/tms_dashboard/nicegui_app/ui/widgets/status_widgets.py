...
def create_status_widgets(dashboard: DashboardState):
    ...
    with ui.column().style('margin-bottom: 5px;'):
        ui.label('Connection').style('font-size: 1.1rem; font-weight: 600; color: #4b5563; margin-bottom: 2px;')
        ...
        with ui.row().style('gap: 2.5rem; flex: 1; align-items: center;'):
            with ui.row().style('gap: 5px;'):
                ...
                label = ui.label('Project').style('font-size: 0.9rem; color: #6b7280;')
            ...
            with ui.row().style('gap: 5px;'):
                ...
                label = ui.label('Robot').style('font-size: 0.9rem; color: #6b7280;')
            ...
            with ui.row().style('gap: 5px;'):
                ...
                label = ui.label('Camera').style('font-size: 0.9rem; color: #6b7280;')
    ...
    with ui.column().style('margin-bottom: 5px;'):
        ui.label('Markers').style('font-size: 1.1rem; font-weight: 600; color: #4b5563; margin-bottom: 2px;')
        ...
        with ui.row().style('gap: 2.5rem; width: 100%;'):
            for label_text, icon in [
                ...
                with ui.row().style('gap: 4px;'):
                    ...
                    label = ui.label(label_text).style('font-size: 0.95rem; color: #6b7280;')
    ...
    with ui.column().style('margin-bottom: 5px;'):
        ui.label('Fiducials').style('font-size: 1.1rem; font-weight: 600; color: #4b5563; margin-bottom: 2px;')
        
        with ui.row().style('gap: 2.5rem;'):
            # Image fiducials (left column)
            with ui.column().style('gap: 1px;'):
                ui.label('Image').style('font-size: 1.0rem; font-weight: 600; color: #6b7280; margin-bottom: 1px;')
                for label_text, icon_name in [
                    ...
                    with ui.row().classes('items-center').style('gap: 2px;'):
                        ...
                        label = ui.label(label_text).style('font-size: 0.9rem; color: #6b7280;')
            ...
            with ui.column().style('gap: 1px;'):
                ui.label('Tracker').style('font-size: 1.0rem; font-weight: 600; color: #6b7280; margin-bottom: 1px;')
                for label_text, icon_name in [
    ...
    with ui.column().style('gap: 1px;'):
        ui.label('Status').style('font-size: 1.1rem; font-weight: 600; color: #4b5563; margin-bottom: 2px;')
        ...
        with ui.row().style('gap: 2.5rem; margin-bottom: 2px; width: 100%;'):
            with ui.row().style('gap: 3px;'):
                ...
                label = ui.label('Target').style(f'font-size: 0.9rem; color: #6b7280;')
            ...
            with ui.row().style('gap: 3px;'):
                ...
                label = ui.label('Coil at target').style('font-size: 0.9rem; color: #6b7280;')
                dashboard.__dict__['icon_coil'] = icon
                dashboard.__dict__['label_coil'] = label
        
            with ui.row().style('gap: 3px;'):
                icon = ui.icon('sync').style('font-size: 22px; color: #9ca3af;')
                label = ui.label('Moving Robot').style('font-size: 0.9rem; color: #6b7280;')
    ...
    with ui.column().style('gap: 1px;'):
        ui.label('Indicators').style('font-size: 1.1rem; font-weight: 600; color: #4b5563; margin-bottom: 2px;')
...
def new_status_widgets(dashboard: DashboardState):
    with ui.row().classes('w-full h-full').style('gap: 0;'):
        with ui.column().classes('h-full').style('flex: 30;align-items: center;'):
            ui.image(str(IMAGES_DIR / 'icones/stylus_icon.png')).classes('rounded').style(
                'width: 140px; height: 140px; object-fit: cover'
            )
            ui.label('Probe').style('font-size: 1.2rem; color: #9ca3af; font-weight: 500;')
            ui.image(str(IMAGES_DIR / 'icones/head_icon.png')).classes('rounded').style(
                'width: 140px; height: 140px; object-fit: cover;'
            )
            ui.label('Head').style('font-size: 1.2rem; color: #9ca3af; font-weight: 500;')
            ui.image(str(IMAGES_DIR / 'icones/coil_no_handle_icon.png')).classes('rounded').style(
                'width: 140px; height: 140px; object-fit: cover;'
            )
            ...
            with ui.row().classes('w-full').style('gap: 2px; flex: 70;'):
                with ui.column().classes('h-full').style('flex: 50; justify-content: center;'):
                    with ui.column().classes('w-full').style('gap: 2px; justify-content: center; align-items: center; flex: 1;'):
                        ui.circular_progress().style('font-size: 80px; color: #9ca3af;')
                        label = ui.label('Force').style('font-size: 1.15rem; color: #9ca3af; font-weight: 500;')
                        dashboard.__dict__['label_force'] = label
                    with ui.column().classes('w-full').style('gap: 2px; justify-content: center; align-items: center; flex: 1;'):
